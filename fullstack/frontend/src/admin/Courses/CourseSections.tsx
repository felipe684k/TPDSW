  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-800">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-sm font-bold text-slate-200">Comisiones del Curso</h3>
        <div className="flex gap-2">
          <select 
            value={selectedAcademicYearId} 
            onChange={handleAcademicYearChange}
            className="border border-slate-700 bg-[#1c1d24] text-slate-200 rounded p-1.5 text-xs outline-none focus:border-indigo-500"
          >
      ...
      {loading ? (
        <div className="text-slate-400 text-xs">Cargando comisiones...</div>
      ) : sections.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sections.map((section) => (
            <div key={section.id_section} className="bg-[#17181e] p-4 rounded-lg border border-slate-700 shadow-sm flex flex-col">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-0.5 mt-0.5">
                      {section.schedules && section.schedules.map(h => (
                        <span key={h.id_schedule} className="text-[10px] font-semibold text-slate-300 bg-slate-800 px-1.5 py-0.5 rounded">
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-start justify-center p-6 overflow-y-auto z-50">
          <div className="bg-[#1c1d24] rounded-xl shadow-2xl w-full max-w-xl flex flex-col overflow-hidden my-auto border border-slate-800">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-[#17181e]">
              ...
              <button onClick={() => setModalOpen(false)} className="text-slate-500 hover:text-slate-300 cursor-pointer">✕</button>
            </div>
