  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-100">Valores de Cuota</h1>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {cuotasRegistradas.map((cuota) => (
                <tr key={cuota.id_cuota} className="hover:bg-[#17181e] transition-colors">
                  <td className="p-3 text-xs font-mono font-bold text-slate-400">#{cuota.id_cuota}</td>
                  <td className="p-3 text-xs text-slate-300 font-semibold">{cuota.descripcion}</td>
                  <td className="p-3 text-xs font-bold text-slate-100">
                    ${cuota.costo_mensual.toLocaleString('es-AR')}
                  </td>
                  <td className="p-3 text-xs font-mono text-slate-500">{cuota.fecha_desde}</td>
                  <td className="p-3 text-xs">
                    <button className="text-indigo-400 hover:text-indigo-300 font-semibold text-2xs mr-2">Editar</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-start justify-center p-6 overflow-y-auto z-50">
          <div className="bg-[#1c1d24] rounded-xl shadow-xl w-full max-w-md max-h-[calc(100vh-48px)] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            
            {/* Header */}
            <div className="p-4 border-b border-slate-800 flex justify-between items-center shrink-0">
              <h2 className="text-sm font-semibold text-slate-200">💰 Registrar Valor Cuota</h2>
              <button 
            ...
            <form className="p-5 space-y-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-400">Descripción / Identificador *</label>
                <input 
              ...
              {/* Costo Mensual */}
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-400">Costo Mensual ($) *</label>
                <input 
              ...
              {/* Fecha Desde */}
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-400">Válido Desde (Fecha Desde) *</label>
                <input 
                  type="date" required 
