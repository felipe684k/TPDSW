  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const h of formData.schedules) {
      if (h.start_time >= h.end_time) {
        setToast({ text: `Invalid schedule on ${h.day}: End time must be after start time.`, type: 'danger' })
    ...
    if (!selectedAcademicYearId) {
      setToast({ text: "Select an academic year first", type: 'danger' })
      return
    }
    ...
    try {
      ...
      setModalOpen(false)
      setToast({ text: "Section created successfully", type: 'success' })
      ...
      setToast({ text: error.message || "Error creating section", type: 'danger' })
      setTimeout(() => setToast(null), 4000)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-100">Sections and Schedules</h1>
          <p className="text-xs text-slate-500 mt-1">Management of classrooms, schedule overlaps, and professor assignments.</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button 
            onClick={() => {
              if(!selectedAcademicYearId) { setToast({text:'You must select or create an academic year first', type:'danger'}); return; }
              setModalOpen(true)
            }} 
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="text-slate-400 text-sm">Loading sections...</div>
        ) : courses.length === 0 ? (
          ...
          courses.map(curso => {
            const courseSections = (sections || []).filter(c => c.id_course === curso.id_course);
            const isExpanded = expandedCourse === curso.id_course;

            return (
              <div key={curso.id_course} className="bg-[#1c1d24] rounded-xl border border-slate-800 shadow-sm overflow-hidden transition-all">
                {/* Course Header Button */}
                <button 
                  onClick={() => setExpandedCourse(isExpanded ? null : curso.id_course!)}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-bold text-slate-100">{curso.course_name}</h3>
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-indigo-950/30 text-indigo-400 text-[10px] font-semibold">
                          {curso.level?.name || `Level ${curso.level_code}`}
                        </span>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">
                        {courseSections.length} {courseSections.length === 1 ? 'section registered' : 'sections registered'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 border-t border-slate-800/60 bg-[#17181e]/30">
                    {courseSections.length === 0 ? (
                      <div className="text-center p-6 text-xs text-slate-500 border border-dashed border-slate-800 rounded-lg">
                        No sections created yet for this course in this academic year.
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                        {courseSections.map((comision) => (
                          <div key={comision.id_section} className="bg-[#1c1d24] p-5 rounded-xl border border-slate-800 shadow-sm flex flex-col hover:border-indigo-500/30 transition-colors">
                            
                            <div className="flex justify-between items-start border-b border-slate-800/60 pb-3 mb-3">
                              <div>
                                <h3 className="text-sm font-bold text-slate-100">{comision.name}</h3>
                              </div>
                            </div>

                            <div className="space-y-3">
                              <div>
                                <span className="text-[10px] text-slate-400 block">👨‍🏫 Responsible Professor</span>
                                <span className="text-xs font-semibold text-slate-300">
                                  {comision.professors && comision.professors.length > 0 
                                    ? `${comision.professors[0].first_name} ${comision.professors[0].last_name}` 
                                    : 'Unassigned'}
                                </span>
                              </div>
                              <div className="grid grid-cols-2 gap-4">
                                <div>
                                  <span className="text-[10px] text-slate-400 block">🏫 Classroom</span>
                                  <span className="text-xs font-semibold text-slate-300">{comision.classroom?.name || '-'}</span>
                                </div>
                                <div>
                                  <span className="text-[10px] text-slate-400 block">⏰ Schedules</span>
                                  <div className="flex flex-col gap-0.5 mt-0.5">
                                    {comision.schedules && comision.schedules.map(h => (
                                      <span key={h.id_schedule} className="text-[10px] font-semibold text-slate-300 bg-slate-900 px-1.5 py-0.5 rounded">
                                        {h.day.substring(0,3)}: {h.start_time.substring(0,5)} to {h.end_time.substring(0,5)}
                                      </span>
                                    ))}
                                  </div>
                                </div>
                            ...
                            <div className="mt-4 pt-3 border-t border-slate-800/60 flex justify-end gap-3">
                              <button onClick={() => handleDelete(comision.id_section!)} className="text-rose-500 hover:text-rose-400 font-semibold text-[10px] cursor-pointer">Delete</button>
                            </div>
                          </div>
                        ))}
                      </div>
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-start justify-center p-6 overflow-y-auto z-50">
          <div className="bg-[#1c1d24] rounded-xl shadow-2xl w-full max-w-xl flex flex-col overflow-hidden my-auto border border-slate-800">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-[#17181e]">
              <h3 className="text-sm font-bold text-slate-100">New Section</h3>
            ...
            <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-400 block mb-1">Course *</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-400 block mb-1">Classroom *</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-400 block mb-1">Assigned Professor *</label>
              ...
              <div className="border border-slate-800 rounded-lg p-4 bg-[#17181e]">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-xs font-semibold text-slate-300">Schedules</span>
              ...
              <div className="flex gap-2 pt-2 border-t border-slate-800">
                <button type="button" onClick={() => setModalOpen(false)} className="flex-1 px-3 py-2 border border-slate-700 bg-transparent hover:bg-slate-800 text-slate-300 rounded text-xs font-medium cursor-pointer">Cancel</button>
                <button type="submit" className="flex-1 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs font-medium cursor-pointer">Validate and Save</button>
              </div>
            </form>
          </div>
        </div>
