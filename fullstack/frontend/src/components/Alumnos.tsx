...
export default function Alumnos() {
  ...
  useEffect(() => {
    ...
    const fetchAlumnosFromBackend = async () => {
      try {
        const urlObj = new URL('http://localhost:3000/api/users')
        if (busqueda) urlObj.searchParams.append('search', busqueda)
        if (nivelFiltro) urlObj.searchParams.append('nivel', nivelFiltro)
          ...
          if (json.data && Array.isArray(json.data)) {
            ...
            if (json.data.length > 0) {
              const mapped = json.data.map((item: any) => ({
                ...
                apellido: item.apellido || item.nombre_usuario || 'Sin Apellido',
                nombre: item.nombre || item.nombre_usuario || 'Sin Nombre',
                telefono: item.telefono || 'N/A',
                email: item.email || 'N/A',
                nivel: item.nivel_actual || 'Pendiente',
