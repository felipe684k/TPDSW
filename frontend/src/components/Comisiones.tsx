    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {comisionesEjemplo.map((comision) => (
          <div key={comision.id_comision} className="bg-[#1c1d24] p-5 rounded-xl border border-slate-800 shadow-sm flex flex-col hover:shadow-md transition-shadow">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-0.5 mt-0.5">
                    {comision.horarios.map((h, i) => (
                      <span key={i} className="text-xs font-semibold text-slate-300">{h}</span>
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="bg-[#1c1d24] rounded-xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            ...
            <form className="p-5 space-y-4">
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-400">Nombre / Identificador de la Comisión *</label>
                <input 
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-slate-400">Horarios Asignados (Seleccione múltiples) *</label>
                <div className="border border-slate-800 rounded p-2 text-xs bg-[#1c1d24] flex flex-col gap-2 max-h-32 overflow-y-auto">
                  {horarios.map(h => (
